...
namespace GameService.ServiceCore
{
    public class ServiceGame
    {
        ...
        public SendGameActionResponse SendGameAction(string command, string userName)
        {
            lock (m_gameLock)
            {
                // First, make sure this player is in the game.
                ...
                if(!foundUser)
                {
                    return SendGameActionResponse.CreateError("This user isn't part of this game.", false);
                }

                // Make sure the game is active.
                if (m_state != KokkaKoroGameState.InProgress)
                {
                    return SendGameActionResponse.CreateError("Game isn't in progress", false);
                }
            }

            // Parse out only the action object.
            try
            {
                int key = command.IndexOf("CommandOptions");
            }
            catch(Exception e)
            {
                Logger.Error("Failed to parse out game action.", e);
                return SendGameActionResponse.CreateError("Failed to parse out GameAction", false);
            }
            return SendGameActionResponse.CreateSuccess();
        }
        ...
        private void StartGameInternal()
        {
            ...
            List<GameLog> actionLog = m_gameEngine.ConsumeAction(null);
